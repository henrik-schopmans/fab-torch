...
def evaluate_aldp(
    ...
    for i in range(n_batches):
        ...
        log_p = log_prob(z)
        log_p_sum = (
            log_p_sum + torch.sum(log_p).detach() - torch.sum(log_det).detach().float()
        )
    ...
    if metric_dir is not None:
        ...
        np.savetxt(
            log_p_path, log_p_hist, delimiter=",", header="it,log_p", comments=""
        )
    ...
    if plot_dir is not None:
        ...
        plt.savefig(
            os.path.join(plot_dir, "ramachandran_%07i.png" % (iter + 1)), dpi=300
        )
        plt.close()
