...
def calculate_test_loss(flow, test_dataloader):
    sum_forward_KL = 0
    batch_idx = 0

    for internal_coords in test_dataloader:
        internal_coords = internal_coords.to("cuda")

        batch_idx += 1

        with torch.no_grad():
            forward_KL = flow._nf_model.forward_kld(internal_coords)

        sum_forward_KL += forward_KL.cpu().item()

    return sum_forward_KL / batch_idx


# Parse input arguments
parser = argparse.ArgumentParser(
    description="Train Boltzmann Generator " "with various objectives"
)
...
parser.add_argument(
    "--resume", action="store_true", help="Flag whether to resume training"
)
parser.add_argument(
    "--tlimit",
    type=float,
...
parser.add_argument(
    "--mode", type=str, default="gpu", help="Compute mode, can be cpu, or gpu"
)
parser.add_argument(
    "--precision",
    type=str,
...
optimizer_name = (
    "adam" if not "optimizer" in config["training"] else config["training"]["optimizer"]
)
...
if optimizer_name == "adam":
    ...
    raise NotImplementedError(
        "The optimizer " + optimizer_name + " is not implemented."
    )
...
lr_warmup = (
    "warmup_iter" in config["training"]
...
max_grad_norm = (
    None
    if not "max_grad_norm" in config["training"]
...
ndim = 60
loss_type = (
    "fab_alpha_div" if "loss_type" not in config["fab"] else config["fab"]["loss_type"]
)
        ...
        if loss_type == "fab_alpha_div":
            ...
            def modified_loss(bs):
                (
                    point_ais,
                    log_w_ais,
                ) = model.annealed_importance_sampler.sample_and_log_weights(bs)
            ...
            def modified_loss(bs):
                ...
                loss = -torch.mean(
                    torch.exp(2 * (log_p_x - log_q_x)).detach() * log_q_x
                )
                return loss
...
if "replay_buffer" in config["training"]:
    min_is_target = (
        min_is_target or config["training"]["replay_buffer"]["type"] == "prioritised"
    )
...
for it in range(start_iter, max_iter):
        ...
        if rb_config["type"] == "uniform":
            if it % rb_config["n_updates"] == 0:
                ...
                (
                    point_ais,
                    log_w_ais,
                ) = model.annealed_importance_sampler.sample_and_log_weights(
                    batch_size, logging=False
                )
                # Filter chirality
                if filter_chirality_train:
                ...
                # Sample from buffer
                buffer_sample = buffer.sample_n_batches(
                    batch_size=batch_size, n_batches=rb_config["n_updates"] - 1
                )
            ...
            if it % rb_config["n_updates"] == 0:
                ...
                # Sample from buffer
                buffer_sample = buffer.sample_n_batches(
                    batch_size=batch_size, n_batches=rb_config["n_updates"]
                )
                buffer_iter = iter(buffer_sample)
            ...
            w_adjust = torch.clip(
                torch.exp(log_w_adjust), max=rb_config["max_adjust_w_clip"]
            )
    ...
    if (it + 1) % log_iter == 0 or it == max_iter - 1:
        ...
        (
            ...
            ais_samples,
            ais_log_w,
        ) = model.annealed_importance_sampler.generate_eval_data(
    ...
    if (it + 1) % checkpoint_iter == 0 or it == max_iter - 1:
        ...
        if lr_scheduler is not None:
            torch.save(
                lr_scheduler.state_dict(), os.path.join(cp_dir, "lr_scheduler.pt")
            )
        if lr_warmup:
            torch.save(
